...
var GitHubSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("GitHub Token").setDesc("GitHub Personal Access Token (Fine-grained PAT recommended)").addText(
      (text) => text.setPlaceholder("github_pat_...").setValue("").onChange(async (value) => {
        if (value) {
          await this.plugin.saveToken(value);
          new import_obsidian.Notice("Token saved securely");
        }
      })
    ).addButton(
      (button) => button.setButtonText("Test Connection").onClick(async () => {
        try {
          ...
          new import_obsidian.Notice("GitHub connection successful!");
        } catch (error) {
          new import_obsidian.Notice(`Connection failed: ${error.message}`);
        }
      })
    );
    new import_obsidian.Setting(containerEl).setName("GitHub Owner").setDesc("The owner of the repository (user or organization)").addText(
      (text) => text.setPlaceholder("username").setValue(this.plugin.settings.owner).onChange(async (value) => {
        this.plugin.settings.owner = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("GitHub Repo").setDesc("The name of the repository").addText(
      (text) => text.setPlaceholder("my-notes").setValue(this.plugin.settings.repo).onChange(async (value) => {
        this.plugin.settings.repo = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Branch").setDesc("The branch to sync with (default: main)").addText(
    ...
    new import_obsidian.Setting(containerEl).setName("Remote Path").setDesc("The path in the repository to sync (empty for root)").addText(
      (text) => text.setPlaceholder("notes").setValue(this.plugin.settings.repoPath).onChange(async (value) => {
        this.plugin.settings.repoPath = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Local Path").setDesc("The sub-path in your Vault to sync (empty for root)").addText(
      (text) => text.setPlaceholder("GitHubSync").setValue(this.plugin.settings.vaultSubPath).onChange(async (value) => {
        this.plugin.settings.vaultSubPath = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Auto Pull on Startup").setDesc("Automatically pull changes from GitHub when Obsidian starts").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.autoPullOnStartup).onChange(async (value) => {
        this.plugin.settings.autoPullOnStartup = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Auto Push on Shutdown").setDesc("Automatically push local changes to GitHub when Obsidian closes").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.autoPushOnShutdown).onChange(async (value) => {
        this.plugin.settings.autoPushOnShutdown = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Sync Markdown Only").setDesc("Only sync .md files (recommended for MVP)").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.syncMarkdownOnly).onChange(async (value) => {
...
var SyncManager = class {
  constructor(app, githubApi, settings, metadataStore, pathFilter, logger, statusBar, conflictResolver) {
    __publicField(this, "app");
    __publicField(this, "githubApi");
    __publicField(this, "settings");
    __publicField(this, "metadataStore");
    __publicField(this, "pathFilter");
    __publicField(this, "logger");
    __publicField(this, "statusBar");
    __publicField(this, "conflictResolver");
    __publicField(this, "dirtyFiles", /* @__PURE__ */ new Set());
    __publicField(this, "isPulling", false);
    __publicField(this, "isPushing", false);
    __publicField(this, "internalWritePaths", /* @__PURE__ */ new Set());
    this.app = app;
  ...
  async pullOnStartup() {
    ...
    try {
      ...
      new import_obsidian2.Notice(`GitHub Sync: Pull failed - ${error.message}`);
    } finally {
      this.isPulling = false;
    }
  }
  async pullFile(remoteFile) {
  ...
  async pushFile(localPath) {
    const localFile = this.app.vault.getAbstractFileByPath(localPath);
    if (!(localFile instanceof import_obsidian2.TFile)) return;
  ...
  mapToLocalPath(remotePath) {
    const relative = remotePath.substring(this.settings.repoPath.length).replace(/^\//, "");
    return (0, import_obsidian2.normalizePath)(this.settings.vaultSubPath + "/" + relative);
  }
  mapToRemotePath(localPath) {
  ...
  async createLocalFile(path, contentBase64) {
  ...
  async updateLocalFile(file, contentBase64) {
...
var MetadataStore = class {
  constructor(plugin) {
    ...
    __publicField(this, "metadataPath");
    this.plugin = plugin;
...
var ConflictResolver = class {
  ...
  async resolvePullConflict(input) {
    ...
    const localConflictPath = (0, import_obsidian3.normalizePath)(`${baseName}.conflict.local.md`);
    const remoteConflictPath = (0, import_obsidian3.normalizePath)(`${baseName}.conflict.remote.md`);
    try {
      ...
      new import_obsidian3.Notice(`Conflict in ${input.path}. Local and remote copies created.`, 1e4);
      ...
      new import_obsidian3.Notice(`Failed to handle conflict for ${input.path}. Check logs.`);
    }
  }
  async resolvePushConflict(input) {
    ...
    try {
      await this.app.vault.create(localConflictPath, input.localContent);
      await this.app.vault.create(remoteConflictPath, input.remoteContent);
      new import_obsidian3.Notice(`Conflict while pushing ${input.path}. Remote changes detected. Conflict copies created.`, 1e4);
...
var MyPlugin = class extends import_obsidian4.Plugin {
  constructor() {
    super(...arguments);
    __publicField(this, "settings");
    __publicField(this, "githubApi", null);
    __publicField(this, "syncManager", null);
    __publicField(this, "metadataStore");
    __publicField(this, "pathFilter");
    __publicField(this, "logger");
    __publicField(this, "statusBar");
    __publicField(this, "conflictResolver");
    __publicField(this, "tokenKey", "github-token");
  }
  async onload() {
